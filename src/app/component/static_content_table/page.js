...
export default function page({ data }) {
  ...
  const Handle_Delete = (id, name) => {
    ...
    axios
      .post(`${api}/user_management/delete_static_content/${id}/`, {}, config)
      .then((res) => {
        console.log("static post delete res ----- ", res.data);
        res.data.status === true
          ? (
          toast.success(`Content Post Delete Successfully`),
          ...
          <div className="relative flex justify-end items-center gap-2">
            <Dropdown className="bg-background border-1 border-default-200">
              ...
              <DropdownMenu>
                <DropdownItem
                  onClick={() => {
                    Handle_Edit(user.id, user.type, user.title, user.content);
                  }}
                >
                  Edit
                </DropdownItem>
  ...
  return (
    <>
      <Modal isOpen={isOpen} onOpenChange={onOpenChange} id={state_id} title={state_title} type={state_type} content={state_content} />
